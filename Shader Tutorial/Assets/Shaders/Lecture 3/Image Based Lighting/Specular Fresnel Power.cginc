...
Interpolators vert (MeshData v)
{
    ...
    o.uv = Rotate(o.uv -0.5, _Time.y *_TimeScale) +0.5;

    //need to pick the mip level
    // 0 is the highest detail
    float height = tex2Dlod(_Height, float4(o.uv, 0, 0)).x * 2 - 1;
    
    //offseting based on the height map
...
float4 frag (Interpolators i) : SV_Target
{
    ...
    //instead of brackets
        ...
        float fresnel = 1- saturate(dot(view, normal));
            ...
            // specular occusion, or specular srength, a way of reducing the intensity of the specular light
            specular +=specularIBL * _SpecularOcculsion * fresnel;
        #endif
